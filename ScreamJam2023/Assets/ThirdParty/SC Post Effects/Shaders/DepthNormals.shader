Shader "Hidden/SC Post Effects/DepthNormals"
{
	...
	float4 AccurateReconstruction(float2 uv)
	{
		...
		return float4(normalize(cross(P2 - P0, P1 - P0)), LINEAR_DEPTH(SAMPLE_DEPTH(uv)));
	}
	...
	float4 Frag(Varyings input) : SV_Target
	{
		...
		#if UNITY_UV_STARTS_AT_TOP && UNITY_VERSION < 202020
		UV = UV * float2(1.0, -1.0) + float2(0.0, 1.0);
		#endif
		
		return AccurateReconstruction(UV);
	}
